...
def test_search(module_scoped_pages_fixture, base_app):
    ...
    pages = Page.search(map_search_params(PageServiceConfig.search, {"sort": "title"}))
    assert pages.items[0].id == 3

    pages = Page.search(
        map_search_params(
            PageServiceConfig.search, {"sort": "title", "sort_direction": "desc"}
        )
    )
    ...
    pages = Page.search(map_search_params(PageServiceConfig.search, {"sort": "url"}))
    assert pages.items[0].id == 3
